...
class RoadworkDetectorNode(Node):
    def __init__(self):
        ...
        self.weather_sub = self.create_subscription(
            String, '/weather/condition', self.weather_callback, 10)
    ...
    def _process_frame(self, cv_image, header):
        ...
        try:
            ...
            wc = self.latest_weather

            overlay = cv_image.copy()
            cv2.rectangle(overlay, (5, 5), (250, 148), (0, 0, 0), -1)
            ...
            # Weather
            if wc is not None:
                weather = wc.get('condition', 'N/A')
                wc_conf = wc.get('confidence', 0)
                wc_color = (0, 255, 0) if weather == 'clear' else (0, 165, 255) if weather == 'fog' else (0, 0, 255)
                cv2.putText(cv_image, f"Weather: {weather} ({wc_conf*100:.0f}%)", (10, 114),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, wc_color, 1)
            else:
                cv2.putText(cv_image, "Weather: N/A", (10, 114),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (150, 150, 150), 1)

            cv2.putText(cv_image, f"Det: {len(detections)} | {processing_time*1000:.0f}ms",
                        (10, 132), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (150, 150, 150), 1)
            ...
            weather_str = ''
            if wc is not None:
                weather_str = f' | Weather: {wc.get("condition", "?")} ({wc.get("confidence", 0)*100:.0f}%)'
            ...
            if d_val is not None and d_val > 1.0 and c_val is not None and c_val > 0.03 and r_val is not None and r_val > 0.03:
                self.get_logger().info(
                    ...
                    f'D: {d_val:.1f}m | C: {c_val*100:.1f}% | R: {r_val*100:.1f}%{road_str}{weather_str}'
                )
            else:
                self.get_logger().info(
                    f'Frame {self.frame_count}: {len(detections)} detections | '
                    f'Processing: {processing_time*1000:.0f}ms | Metrics: N/A{road_str}{weather_str}'
                )
